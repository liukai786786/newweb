			...
			.extend(
					a.jqx._jqxSplitter.prototype,
					{
						defineInstance : function() {
							...
							this.panel1 = null;
							//按钮的风格
							this.splitBarStyle=null;
							//按钮的位置
							this.splitBarPosition=null;
							this.panel2 = null;
							this._eventsMap = {
							...
							this._events = [ "resize", "expanded", "collapsed",
									"resizeStart", "layout" ]
						},
						...
						_addHandlers : function() {
							...
							if (this.splitBarButton) {
								...
								this.addHandler(this.splitBarButton,
												this._eventName("mouseenter"),
												function() {//alert(event.srcElement.outerHTML);
								...
								if( c.splitBarButton.length> 0){
								   if($.type(this.splitBarStyle) == "string"){
								   c.splitBarButton.addClass(this.splitBarStyle);
								   }
								} 				
							}
							...
							this.addHandler(this.splitBar,
											this._eventName("mouseenter"),
											function() {if (c.resizable && !c.disabled) {
						...
						_arrange : function() {
							...
							//wanhm2014/1/16
							var $v = this.splitBar;
							this.splitBar.css("overflow","hidden");
							//this.splitBar.css("padding-top","1px");
							//alert($v.get(0).offsetHeight);
							//alert(parseInt(this.splitBar.css("height"))/2);
							...
							this.splitBarButton.css("position", "absolute");		
							if (this.orientation === "vertical") {
								//whm add 2014/1/16
								 
								//splitBarPosition
								var top_ = Math.round(parseInt(this.splitBar.css("height"))/2); 
								if($.type(this.splitBarPosition) == 'object'){ 
									this.splitBarButton.css(this.splitBarPosition);
								}else{
									this.splitBarButton.css("top", top_);
								} 
								//this.splitBarButton.css("top", "-23px");
