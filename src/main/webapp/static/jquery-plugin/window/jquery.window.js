...
$.Window = (function()  {
	...
	var ICON_WH = 106;               // window icon button width/height, in pixels. check "window_icon_button" style in css
	var ICON_MARGIN = 1;            // window icon button margin, in pixels. check "window_icon_button" style in css
	...
	function pushMinWindow(parent, wnd) {
		var aElementTmp = [];
		$.each(aElement,function(index,obj){
	   		if(obj.getWindowId() == wnd.getWindowId()){

	   		}else{
	   		aElementTmp.push(obj);
	   		}
	   });
	   aElement = aElementTmp;
	   windowsArray.Push(wnd);
	   	showCurrentOpenWindowList();
	...
	function constructor(caller, options) {
		...
		var windowId = "window_" +options.winId; // the window's id
		var iconPath = options.iconPath;
		var winName = options.name;
		...
		//alert(options.winId+options.iconPath+options.name);
		...
		function initialize(instance) {
			...
			var headerHtml = "<div class='window_header window_header_normal "+cornerClass+"no-resizable "+options.headerClass+"'>"+
				iconHtml+
			...
			//header.append("<img src='./images2/001.png' style='vertical-align:middle;width:16px;height:16px;position:absolute;top:10px;' />");
			headerFuncPanel = header.children("div.window_function_bar");
			// add close button
			if( options.closable ) {
				headerFuncPanel.append( "<div title='返回系统首页' class='closeImg window_icon_button no-draggable'></div>" );
				headerFuncPanel.children(".closeImg").click(function() {
					close();
				});
				headerFuncPanel.children(".closeImg").hover(function(){
					headerFuncPanel.children(".closeImg").css({
					background:'url(/static/jquery-plugin/window/img/close_highlight.png) no-repeat'	
					});
				},function(){
					headerFuncPanel.children(".closeImg").css({
					background:'url(/static/jquery-plugin/window/img/close.png) no-repeat'	
					});
				});
			...
			if( options.maximizable ) {
				...
				headerFuncPanel.append( "<div title='恢复窗口' class='cascadeImg window_icon_button no-draggable' style='display:none;'></div>" );
			...
			if( options.minimizable ) {
				headerFuncPanel.append( "<div title='最小化当前页' class='minimizeImg window_icon_button no-draggable'></div>" );
				headerFuncPanel.children(".minimizeImg").click(function() {
					alert("功能未实现");
					//minimize();
				});
				headerFuncPanel.children(".minimizeImg").hover(function(){
					headerFuncPanel.children(".minimizeImg").css({
					background:'url(/static/jquery-plugin/window/img/minimize_highlight.png) no-repeat'	
					});
				},function(){
					headerFuncPanel.children(".minimizeImg").css({
					background:'url(/static/jquery-plugin/window/img/minimize.png) no-repeat'	
					});
				});
				funcBarWidth += ICON_OFFSET;
			}
			...
			if( options.bookmarkable && options.url != null && $.trim(options.url) != "" ) {
				headerFuncPanel.append( "<div title='收藏此页面' class='bookmarkImg window_icon_button no-draggable'></div>" );
				headerFuncPanel.children(".bookmarkImg").click(function() {
					//doBookmark('收藏此页面', options.url);
					alert('收藏页面');
				});
								headerFuncPanel.children(".bookmarkImg").hover(function(){
					headerFuncPanel.children(".bookmarkImg").css({
					background:'url(/static/jquery-plugin/window/img/bookmark_highlight.png) no-repeat'	
					});
				},function(){
					headerFuncPanel.children(".bookmarkImg").css({
					background:'url(/static/jquery-plugin/window/img/bookmark.png) no-repeat'	
					});
				});
				funcBarWidth += ICON_OFFSET;
			}
			...
			if( options.url != null && $.trim(options.url) != "" ) {
				...
				// show loading image
				container.append("<div class='frame_loading' style='height:100px;'>Loading...</div>");
		...
		function restoreTitleText() {
			...
			textBlock.addClass('window_title_text_self');
			textBlock.text(options.name);
		}
		...
		function minimize() {
			options.nameTmp = options.title;
			options.title=options.name;
			//header.children("div.window_title_text").html(options.name);
			hideOverlay(true);
		...
		function restore() {
			...
			options.title = options.name;
			//header.children("div.window_title_text").html(options.title);
			if( options.showModal ) {
				showOverlay();
			}
			var rpos = null;
		...
		function close(quiet) {
			//var tmpWindowArray =[];
			//$.each(aElement,function(index,obj){
	   		//if(obj.getWindowId() == 'window_'+options.winId){
	   	
	   		//}else{
	   		//tmpWindowArray.push(obj);
	   		//}
	  // });
	   //aElement = tmpWindowArray;
	  // showCurrentOpenWindowList();
			// do callback
